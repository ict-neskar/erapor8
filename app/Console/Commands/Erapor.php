...
class Erapor extends Command
{
                        ...
                        if($response){
                            if($response->message){
                                ...
                                if ($this->option('force')){
                                    ...
                                    SyncLog::updateOrCreate(['user_id' => $user->user_id, 'updated_at' => now()]);
                                } else {
                                    foreach($table_sync as $table){
    ...
    private function proses_kirim($user_id, $table, $sekolah_id, $tahun_ajaran_id, $semester_id){
        ...
        SyncLog::updateOrCreate(['user_id' => $user_id, 'updated_at' => now()]);
    }
    private function kirim_data($user_id, $table, $data, $sekolah_id, $tahun_ajaran_id, $semester_id){
        ...
        if($response->status){
            if($this->argument('akses')){
                $this->call('app:respon-artisan', ['status' => 'info', 'title' => 'Berhasil', 'respon' => count($data).' data '.nama_table($table).' berhasil dikirim']);
            }
            $this->info(count($data).' data '.nama_table($table). ' berhasil dikirim');
            $this->update_last_sync($user_id, $table, $data, $sekolah_id);
        } else {
            if($this->argument('akses')){
                $this->call('app:respon-artisan', ['status' => 'error', 'title' => 'Gagal', 'respon' => 'Proses pengiriman data '.nama_table($table).' gagal. Server tidak merespon. Status Server: '.$response->message]);
            }
    ...
    private function getTableForce($user_id, $table, $sekolah_id, $tahun_ajaran_id, $semester_id){
        ...
        if($jml){
            ...
            $getData->orderBy('created_at')->chunk(500, function ($data) use ($bar, $user_id, $table, $sekolah_id, $tahun_ajaran_id, $semester_id){
                foreach($data as $d){
                    $bar->advance();
                }
                try {
