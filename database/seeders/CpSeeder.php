    ...
    public function run(): void
    {
        ...
        foreach($data as $d){
            ...
            foreach($d['elemen'] as $elemen){
                ElemenBudayaKerja::updateOrCreate(
                    [
                        'elemen_id' => $elemen['id'],
                    ],
                    [
        ...
        foreach($data as $d){
            OpsiBudayaKerja::updateOrCreate(
                [
                    'opsi_id' => $d['id'],
                ],
                [
        ...
        $data = (new FastExcel)->import(public_path('templates/ref_cp.xlsx'), function ($line) {
            $mapel = MataPelajaran::find($line['mata_pelajaran_id']);
            if($mapel){
                $find = KompetensiDasar::where(function($query) use ($line){
        ...
        KompetensiDasar::where('kurikulum', 2022)->forceDelete();
        $data = (new FastExcel)->import(public_path('templates/ref_cp_2.xlsx'), function ($line) {
            ...
            if($line['no'] && $line['mata_pelajaran_id']){
                ...
                if(CapaianPembelajaran::where('cp_id', $line['no'])->delete()){
        ...
        $data = (new FastExcel)->import(public_path('templates/ref_cp_pkl.xlsx'), function ($line) {
            $mapel = MataPelajaran::find($line['mata_pelajaran_id']);
            if($mapel){
                $count = CapaianPembelajaran::orderBy('cp_id', 'DESC')->first();
                if($count){
                    $id = $count->cp_id + 1;
                    CapaianPembelajaran::updateOrCreate(
                        [
